"""
src/team_catboost_test6.py
──────────────────────────
[B] QA 역문항 리버스 코딩과 [C] 역문항 보정 후 계산 부분을 제외하고, TP 결측치 처리(NaN 변환) 및 관련 파생 피처 생성만 유지
"""
...
def prepare(train_df, test_df):
    ...
    # [B] QA 역문항 리버스 코딩 부분 삭제됨 (요청 반영)

    # [C] QA 관련 파생 피처 (역문항 보정 없이 원본 데이터 기반으로 계산)
    avail_q = [c for c in Q_COLS if c in X_train.columns]
    for df in [X_train, X_test]:
        # 역문항 처리를 하지 않은 raw 데이터 그대로 평균, 표준편차 등을 계산합니다.
...
save_npy(oof_pred,  OUTPUT_DIR / "team_catboost_test6_oof.npy")
save_npy(test_pred, OUTPUT_DIR / "team_catboost_test6_test.npy")

try_log(
    "catboost", oof_auc, CATBOOST_PARAMS, {},
    notes=(
        "team_catboost_test6 [vs team_catboost_test5: 역문항보정 제거, tp NaN만 유지] "
        "사용컬럼: QA_raw20+tp_raw10+wr_raw13+wf_raw3+인구통계8개 | "
        "파생: delay_root10+tp_notapplicable_cnt+tp_missing_cnt | "
        "       mach_score+q_response_std+q_extreme_ratio+vocab4개(역문항보정없이raw기반) | "
        "변경A: tp(0→NaN+7→NaN)+카운트2개 | "
        "제거B: 역문항리버스코딩없음 | "
        "변경C: mach_score+vocab4개(역문항보정없이원본raw기반계산) | "
