...
WR/WF SUMMARY 생성 및 표준화, pos_weight 계산 등은 fold 내부에서 수행하여 정보 유출 방지
현재 최고 기록: OOF AUC ≈ 0.7749
...
NN_PARAMS = {
    "architecture": "Dropout(0.05)->180->LeakyReLU->Dropout(0.50)->32->ReLU->1",
    "n_repeat": N_REPEAT,
    "n_skfold": N_SKFOLD,
    "n_epoch": N_EPOCH,
    "batch_size": BATCH_SIZE,
...
try_log(
    "nn", oof_auc, NN_PARAMS, {},
    notes=(
        "team_nn1 [fold내부 WR/WF 처리로 데이터 누수 방지] "
        "사용컬럼: QA_raw20+tp_raw10+wr_raw13+wf_raw3+인구통계(hand제외) | "
        "파생: delay_root10+A_mean+A_std+A_extreme_ratio+A_neutral_ratio | "
        "구조: 5repeat×7fold×48epoch | RefNN: 180→32→1 | "
        "AdamW lr=5e-3 wd=7.8e-2 | CosineAnnealingWarmRestarts T_0=8"
    ),
)
show_leaderboard()
