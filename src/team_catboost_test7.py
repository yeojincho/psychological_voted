...
save_npy(oof_pred,  OUTPUT_DIR / "team_catboost_test7_oof.npy")
save_npy(test_pred, OUTPUT_DIR / "team_catboost_test7_test.npy")

try_log(
    "catboost", oof_auc, CATBOOST_PARAMS, {},
    notes=(
        "team_catboost_test7 [vs test6: QA summary 전체 제거] "
        "사용컬럼: QA_raw20+tp_raw10+wr_raw13+wf_raw3+인구통계8개 | "
        "파생: delay_root10+tp_notapplicable_cnt+tp_missing_cnt+vocab4개 | "
        "변경A: tp(0→NaN+7→NaN)+카운트2개 | "
        "제거: 역문항리버스코딩없음+mach_score+q_response_std+q_extreme_ratio 전부제거 | "
