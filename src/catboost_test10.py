"""
src/catboost_test10.py
──────────────────────
CatBoost 실험 #10 — 팀 공통 FE 컬럼 정의 적용 + 신규 파생 추가

test9 대비 변경:
  [정의] 팀 공통 FE 컬럼 스타일로 통일 (실제 데이터 기준)
       QUESTION_KEYS = list("abcdefghijklmnopqrst")
       QA_COLS = [f"Q{k}A" for k in QUESTION_KEYS]
       QE_COLS = [f"Q{k}E" for k in QUESTION_KEYS]
       TP_COLS = [f"tp0{i}" for i in range(1,10)] + ["tp10"]
       WR_COLS = wr_01~wr_13 (실제 데이터 기준)
       WF_COLS = wf_01~wf_03 (실제 데이터 기준)

  [추가] BigFive 차원별 tp 평균 (OCEAN 5개)
       → tp 항목을 Big5 성격 차원으로 묶어 각 성격 점수화
       → E(외향성): tp01,tp06 / A(친화성): tp02,tp07
       → C(성실성): tp03,tp08 / N(신경성): tp04,tp09
       → O(개방성): tp05,tp10

  [추가] QE 피로도 피처 (qe_first5_mean, qe_last5_mean, qe_fatigue)
       → 설문 초반(QaE~QeE) vs 후반(QpE~QtE) 응답 시간 비교
       → qe_fatigue = 후반5개 평균 / (전반5개 평균 + 1)
         (1보다 크면 후반으로 갈수록 느려짐 → 피로/집중력 저하)

  [유지] test9의 전략적 컬럼 선택 그대로
       hand 제거, QE→delay_root10, 인구통계 전부, raw 유지 위에 파생 추가

실행:
    python -m src.catboost_test10

출력 (신기록 시에만):
    outputs/catboost_test10_submission.csv
"""

import os
import random
import numpy as np
import pandas as pd
from sklearn.model_selection import StratifiedKFold
from sklearn.metrics import roc_auc_score
from catboost import CatBoostClassifier, Pool

from src.tracker import try_log, show_leaderboard

# ================================================================
# 설정 — 팀 공통 FE 컬럼 정의 (실제 데이터 기준)
# ================================================================
TRAIN_PATH = "data/train.csv"
TEST_PATH  = "data/test_x.csv"
OUT_PATH   = "outputs/catboost_test10_submission.csv"
SEED       = 42
N_SPLITS   = 5

QUESTION_KEYS = list("abcdefghijklmnopqrst")
QA_COLS = [f"Q{k}A" for k in QUESTION_KEYS]          # QaA~QtA (20개)
QE_COLS = [f"Q{k}E" for k in QUESTION_KEYS]          # QaE~QtE (20개)
TP_COLS = [f"tp0{i}" for i in range(1, 10)] + ["tp10"]  # tp01~tp10 (10개)
WR_COLS = [f"wr_{str(i).zfill(2)}" for i in range(1, 14)]  # wr_01~wr_13 (실제 데이터)
WF_COLS = [f"wf_{str(i).zfill(2)}" for i in range(1, 4)]   # wf_01~wf_03 (실제 데이터)

REVERSE_Q = ["QaA","QdA","QeA","QfA","QgA","QiA","QkA","QnA","QqA","QrA"]

# hand 제거 — 나머지 8개 유지
CAT_COLS = ["age_group","education","engnat","gender",
            "married","race","religion","urban"]

# Big5 차원별 tp 항목 (OCEAN)
BIG5_OCEAN = {
    "tp_E": ["tp01","tp06"],   # Extraversion (외향성)
    "tp_A": ["tp02","tp07"],   # Agreeableness (친화성)
    "tp_C": ["tp03","tp08"],   # Conscientiousness (성실성)
    "tp_N": ["tp04","tp09"],   # Neuroticism (신경성)
    "tp_O": ["tp05","tp10"],   # Openness (개방성)
}

# ================================================================
# 하이퍼파라미터 (test9와 동일)
# ================================================================
PARAMS = dict(
    objective        = "Logloss",
    eval_metric      = "AUC",
    iterations       = 3000,
    learning_rate    = 0.03,
    depth            = 6,
    l2_leaf_reg      = 5,
    bootstrap_type   = "Bernoulli",
    subsample        = 0.8,
    scale_pos_weight = 1.20665,
    random_seed      = SEED,
    allow_writing_files = False,
)
FIT_PARAMS = dict(
    early_stopping_rounds = 200,
    verbose               = 200,
)


# ================================================================
# 피처 엔지니어링
# ================================================================
def preprocess(train_raw: pd.DataFrame, test_raw: pd.DataFrame):
    train, test = train_raw.copy(), test_raw.copy()

    for df in [train, test]:
        if "index" in df.columns:
            df.drop(columns=["index"], inplace=True)

    # hand 제거
    for df in [train, test]:
        df.drop(columns=["hand"], errors="ignore", inplace=True)

    # familysize 이상치 제거 (train만)
    outlier_idx = train[train["familysize"] > 50].index
    if len(outlier_idx):
        print(f"familysize 이상치 제거: {len(outlier_idx)}행")
        train = train.drop(index=outlier_idx).reset_index(drop=True)

    # ── 1. tp 파생 피처 (NaN 변환 전) ───────────────────────────
    avail_tp = [c for c in TP_COLS if c in train.columns]
    for df in [train, test]:
        df["tp_notapplicable_cnt"] = (df[avail_tp] == 7).sum(axis=1)
        df["tp_missing_cnt"]       = (df[avail_tp] == 0).sum(axis=1)
    for col in avail_tp:
        train[col] = train[col].replace({0: np.nan, 7: np.nan})
        test[col]  = test[col].replace({0: np.nan, 7: np.nan})

    # tp 일관성 피처 (NaN 변환 후)
    for df in [train, test]:
        df["tp_mean"]  = df[avail_tp].mean(axis=1)
        df["tp_std"]   = df[avail_tp].std(axis=1)
        df["tp_range"] = df[avail_tp].max(axis=1) - df[avail_tp].min(axis=1)

    # [NEW] Big5 차원별 평균 (NaN 변환 후)
    for dim_name, dim_cols in BIG5_OCEAN.items():
        avail_dim = [c for c in dim_cols if c in train.columns]
        if avail_dim:
            for df in [train, test]:
                df[dim_name] = df[avail_dim].mean(axis=1)

    # ── 2. 인구통계 무응답 → NaN ─────────────────────────────────
    for col in ["education","engnat","married","urban"]:
        if col in train.columns:
            train[col] = train[col].replace(0, np.nan)
            test[col]  = test[col].replace(0, np.nan)

    # ── 3. 역문항 리버스 코딩 ────────────────────────────────────
    for col in REVERSE_Q:
        if col in train.columns:
            train[col] = 6 - train[col]
            test[col]  = 6 - test[col]

    # ── 4. MACH 파생 (raw 위에 추가) ─────────────────────────────
    avail_q = [c for c in QA_COLS if c in train.columns]
    for df in [train, test]:
        df["mach_score"]      = df[avail_q].mean(axis=1)
        df["q_response_std"]  = df[avail_q].std(axis=1)
        df["q_extreme_ratio"] = (
            (df[avail_q] == 1) | (df[avail_q] == 5)
        ).sum(axis=1) / len(avail_q)

    # ── 5. 어휘력 파생 (raw 위에 추가) ───────────────────────────
    avail_wr = [c for c in WR_COLS if c in train.columns]
    avail_wf = [c for c in WF_COLS if c in train.columns]
    for df in [train, test]:
        df["vocab_real"]     = df[avail_wr].sum(axis=1)
        df["vocab_fake"]     = df[avail_wf].sum(axis=1)
        df["vocab_score"]    = df["vocab_real"] - df["vocab_fake"] * 2
        df["vocab_accuracy"] = df["vocab_real"] / (df["vocab_real"] + df["vocab_fake"] + 1e-6)

    # ── 6. QE → delay_root10 + [NEW] 피로도 피처 ─────────────────
    avail_qe = [c for c in QE_COLS if c in train.columns]
    first5   = avail_qe[:5]   # QaE~QeE (설문 초반)
    last5    = avail_qe[-5:]  # QpE~QtE (설문 후반)
    for df in [train, test]:
        qe_vals = df[avail_qe].clip(lower=0)
        df["delay_root10"]   = np.power(qe_vals.sum(axis=1), 0.1)
        # [NEW] 피로도
        df["qe_first5_mean"] = df[first5].clip(lower=0).mean(axis=1)
        df["qe_last5_mean"]  = df[last5].clip(lower=0).mean(axis=1)
        df["qe_fatigue"]     = df["qe_last5_mean"] / (df["qe_first5_mean"] + 1)
    train = train.drop(columns=avail_qe)
    test  = test.drop(columns=avail_qe)

    # ── 7. familysize log1p ──────────────────────────────────────
    for df in [train, test]:
        if "familysize" in df.columns:
            df["familysize"] = np.log1p(df["familysize"].clip(lower=0))

    # ── 8. 범주형 → 문자열 (CatBoost용) ─────────────────────────
    for col in CAT_COLS:
        if col in train.columns:
            train[col] = train[col].astype(str)
            test[col]  = test[col].astype(str)

    return train, test


# ================================================================
# CV 학습
# ================================================================
def train_cv(X_train, y_train, X_test, cat_idx):
    skf        = StratifiedKFold(n_splits=N_SPLITS, shuffle=True, random_state=SEED)
    oof_preds  = np.zeros(len(X_train))
    test_preds = np.zeros(len(X_test))

    for fold, (tr_idx, val_idx) in enumerate(skf.split(X_train, y_train), 1):
        print(f"\n── Fold {fold}/{N_SPLITS} ──────────────────────────")

        train_pool = Pool(X_train.iloc[tr_idx],  label=y_train[tr_idx],  cat_features=cat_idx)
        val_pool   = Pool(X_train.iloc[val_idx], label=y_train[val_idx], cat_features=cat_idx)
        test_pool  = Pool(X_test, cat_features=cat_idx)

        model = CatBoostClassifier(**PARAMS, **FIT_PARAMS)
        model.fit(train_pool, eval_set=val_pool, use_best_model=True)

        vp = model.predict_proba(val_pool)[:, 1]
        oof_preds[val_idx] = vp
        test_preds += model.predict_proba(test_pool)[:, 1] / N_SPLITS

        print(f"Fold {fold} AUC: {roc_auc_score(y_train[val_idx], vp):.5f}")

    oof_auc = roc_auc_score(y_train, oof_preds)
    print(f"\n[OOF AUC] {oof_auc:.5f}")
    return oof_preds, test_preds, oof_auc


# ================================================================
# Main
# ================================================================
def main():
    random.seed(SEED)
    np.random.seed(SEED)

    train_raw  = pd.read_csv(TRAIN_PATH)
    test_raw   = pd.read_csv(TEST_PATH)
    test_index = test_raw["index"] if "index" in test_raw.columns \
                 else pd.RangeIndex(len(test_raw))

    train_raw["voted"] = (train_raw["voted"] == 1).astype(int)
    train_proc, test_proc = preprocess(train_raw, test_raw)

    y_train = train_proc["voted"].values
    X_train = train_proc.drop(columns=["voted"])
    X_test  = test_proc.drop(columns=["voted"], errors="ignore")

    common  = [c for c in X_train.columns if c in X_test.columns]
    X_train, X_test = X_train[common], X_test[common]

    cat_idx = [i for i, c in enumerate(X_train.columns) if c in CAT_COLS]

    print(f"피처: {X_train.shape[1]}개 | 범주형: {len(cat_idx)}개 | 샘플: {len(y_train)}")
    print(f"추가: BigFive(tp_E/A/C/N/O) + QE피로도(qe_first5/last5/fatigue)\n")

    oof_preds, test_preds, oof_auc = train_cv(X_train, y_train, X_test, cat_idx)

    os.makedirs("outputs", exist_ok=True)
    recorded = try_log(
        "catboost", oof_auc, PARAMS, FIT_PARAMS,
        notes=(
            "catboost_test10 [팀공통FE컬럼정의+BigFive+QE피로도] "
            "사용컬럼: QA_raw20+tp_raw10+wr_raw13+wf_raw3+인구통계8개(hand제거) | "
            "파생(test9유지): mach_score+q_response_std+q_extreme_ratio+vocab4개 | "
            "       delay_root10+tp_mean+tp_std+tp_range+tp_notapplicable_cnt+tp_missing_cnt | "
            "추가파생: tp_E+tp_A+tp_C+tp_N+tp_O(BigFive차원별평균) | "
            "       qe_first5_mean+qe_last5_mean+qe_fatigue(QE피로도) | "
            "컬럼정의: 팀공통FE기준(WR=wr_01~13,WF=wf_01~03실제데이터) | "
            "하이퍼파라미터: test9와동일(depth=6+lr=0.03+l2=5)"
        ),
    )
    if recorded:
        pd.DataFrame({"index": test_index, "voted": test_preds}).to_csv(OUT_PATH, index=False)
        print(f"submission → {OUT_PATH}")
    else:
        print("[tracker] 신기록 아님 → submission 저장 생략")

    show_leaderboard()


if __name__ == "__main__":
    main()
