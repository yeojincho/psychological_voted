...
src/lgbm_test6.py
─────────────────
LightGBM 실험 #6 — 새로운 파생 피처 추가 (C)

팀 공통 FE 컬럼 정의 기준 (실제 데이터 컬럼명).

lgbm_test4 대비 추가:
  [추가] QE 피로도 피처 (qe_first5_mean, qe_last5_mean, qe_fatigue)
       → 설문 초반 vs 후반 응답 시간 비교
       → qe_fatigue = (후반5개 평균) / (전반5개 평균 + 1)
         (1보다 크면 후반으로 갈수록 느려짐 → 피로/집중력 저하)
  [추가] tp BigFive 차원별 평균 (OCEAN)
       → tp 항목을 Big5 차원으로 묶어 각 성격 특성 점수화
       → O(개방성): tp05,tp10 / C(성실성): tp03,tp08
       → E(외향성): tp01,tp06 / A(친화성): tp02,tp07
       → N(신경성): tp04,tp09
  [추가] vocab_wr_ratio (실제단어 인식률: vocab_real / wr 문항수)
  [추가] mach_high_flag (mach_score 상위 40% binary — LGBM category로)

실행:
    python -m src.lgbm_test6
"""
...
OUT_PATH   = "outputs/lgbm_test6_submission.csv"
SEED       = 42
N_SPLITS   = 5
...
BIG5_OCEAN = {
    "tp_E": ["tp01","tp06"],   # Extraversion
    "tp_A": ["tp02","tp07"],   # Agreeableness
    "tp_C": ["tp03","tp08"],   # Conscientiousness
    "tp_N": ["tp04","tp09"],   # Neuroticism
    "tp_O": ["tp05","tp10"],   # Openness
}
...
CROSS_COLS   = ["age_x_edu","gender_x_urban","race_x_religion",
                "married_x_urban","gender_x_edu"]
BIN_COLS     = ["mach_bin","vocab_bin","mach_high_flag"]
...
def preprocess(train_raw: pd.DataFrame, test_raw: pd.DataFrame):
    ...
    for df in [train, test]:
        ...
        # [NEW] 실제단어 인식률
        df["vocab_wr_ratio"]  = df["vocab_real"] / (len(avail_wr) + 1e-6)

    # 6. QE → delay + 분산 + [NEW] 피로도
    ...
    # [NEW] mach_high_flag: train 상위 40% 기준 binary category (누수 방지)
    threshold = train["mach_score"].quantile(0.6)
    for df in [train, test]:
        df["mach_high_flag"] = (df["mach_score"] >= threshold).astype(int).astype("category")
...
def main():
    ...
    print(f"      vocab_wr_ratio+mach_high_flag\n")
    ...
    recorded = try_log(
        ...
        notes=(
            "lgbm_test6 [test4에서 새 파생 피처 추가] "
            "사용컬럼: QA_raw20+tp_raw10+wr_raw13+wf_raw3+인구통계8개(hand제거) | "
            "파생(test4유지): mach_score+vocab4개+delay_root10+qe_std/cv/max_ratio | "
            "       tp_mean+tp_std+tp_range+cross_cat5개+bin2개+rank4개 | "
            "추가파생: tp_E+tp_A+tp_C+tp_N+tp_O(BigFive차원별평균) | "
            "       qe_first5_mean+qe_last5_mean+qe_fatigue(QE피로도) | "
            "       vocab_wr_ratio(실제단어인식률)+mach_high_flag(상위40%binary_cat) | "
            "누수방지: mach_high_flag threshold=train.quantile(0.6)"
        ),
    )
    if recorded:
