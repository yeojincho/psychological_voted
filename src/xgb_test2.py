...
src/xgb_test2.py
────────────────
XGBoost 실험 #2 — XGBoost 특성 맞춤 FE 강화

xgb_test1 대비 변경:
  [추가] 카테고리 교차 피처 (age_x_edu, gender_x_urban, race_x_religion)
       → XGBoost는 OrdinalEncoder 후 단순 수치 분기 → 교차항 명시 필요
  ...
  [제거] hand (도메인 무관 노이즈)
  [조정] 하이퍼파라미터: n_estimators 3000→4000, lr 0.03→0.02,
         max_depth 6→7, colsample_bytree 0.8→0.7

실행:
    python -m src.xgb_test2

출력 (신기록 시에만):
    outputs/xgb_test2_submission.csv
"""
...
OUT_PATH   = "outputs/xgb_test2_submission.csv"
SEED       = 42
N_SPLITS   = 5
...
def preprocess(train_raw: pd.DataFrame, test_raw: pd.DataFrame):
    ...
    # ── 8. [NEW] 카테고리 교차 피처 (문자열 → OrdinalEncoder) ────
    for df in [train, test]:
...
def main():
    ...
    print(f"추가 피처: age_x_edu / gender_x_urban / race_x_religion (교차→OrdinalEnc)")
